...
def uid_predict_f_c_l(uid_weibo_dict):
    ...
    uid_l_min_df = pd.read_csv(user_like_min_path,header=None,names=['uid','l_max'],index_col=0)
    ...
    for uid in all_uid_list:
        f_max = uid_f_max_dict[uid].value
        f_min = uid_f_min_dict[uid].value
        best_score_f = 0
        ...
        best_score_c=0
        c_max = uid_c_max_dict[uid].value
        c_min = uid_c_min_dict[uid].value
        ...
        best_score_l = 0
        l_max = uid_l_max_dict[uid].value
        l_min = uid_l_min_dict[uid].value
